    ...
    public SendMessage handleUpdate(Update update, User user)
    {
        ...
        switch (update.getMessage().getText())
        {
            ...
            case "Instagram":
                ...
                {
                    message.setText("Instagram");
                }
                else
                {
                    List<InstaAd> vkAdList = mappers.getInstaAdMapper().GetAll();
                    InstaAd cur = null;
                    for (InstaAd instaAd : vkAdList)
                    {
                        if (mappers.getInstaAdRelationshipMapper().findByIds(user.getTelegramId(), user.getInstagramId()) == null)
                        {
                            cur = instaAd;
                            break;
                        }
            ...
            case "Vk":
                ...
                else
                {
                    ...
                    {
                        if (mappers.getVkAdRelationshipMapper().findByIds(vkAd.getVkGroupId(), user.getTelegramId()) == null)
                        {
                            cur = vkAd;
                            break;
                        }
                    ...
                    else
                    {
                        try
                        {
                            ...
                            VkAdRelationship relationship = new VkAdRelationship(user.getTelegramId(), cur.getVkGroupId(), "Processing");
